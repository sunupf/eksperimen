...
class Test
  ...
  def run
    ...
    wait.until { @driver.find_element(:css => '#sub-activity-list li').displayed? }

    puts @driver.find_element(:css => '#sub-activity-list li').displayed?

    target = @testCase['testCases']['target'].split("#")
    url = target.first
    actionCheck = target.last
    id = url.gsub("http://todo.app/subactivity/","").gsub("/check","")

    puts id;


    wait.until {
      @driver.find_element(:css => "#sub-activity-list>li").displayed?
    }

    begin
      check = @driver.find_element :css => "#sub-activity-#{id} a.check"
    rescue
      check = @driver.find_element :css => "#sub-activity-list>li:first-child>a.check"
